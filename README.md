...
To update the database use this command from the Web project folder (replace `Okala.Exchange` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Okala.Exchange.Infrastructure/Okala.Exchange.Infrastructure.csproj -s Okala.Exchange.Web.csproj
```
